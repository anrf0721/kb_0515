...
# key: task_id
# value: 任务到达终态（completed/failed）的时间戳，用于 TTL 清理
_tasks_finish_time: Dict[str, float] = {}

# 任务到达终态后，数据保留的秒数（10 分钟）
...
def add_running_task(task_id: str, node_name: str) -> None:
    ...
    # 3. 状态兜底：若状态从未设置过，默认置为 processing，
    #    避免任务刚创建到 update_task_status 之间的真空期，前端轮询拿到空状态
    if not _tasks_status.get(task_id):
...
def update_task_status(task_id: str, status_name: str) -> None:
    ...
    # 到达终态时记录完成时间戳，供 TTL 清理使用
    if status_name in (TASK_STATUS_COMPLETED, TASK_STATUS_FAILED):
        _tasks_finish_time[task_id] = time.time()


def cleanup_expired_tasks(ttl_seconds: float = TASK_TTL_SECONDS) -> int:
    """
    清理超过 TTL 的已终态任务数据，防止内存只增不减。
    建议在每次新任务开始时（如 /upload 入口）调用一次。

    返回：清理掉的任务数量
    """
    now = time.time()
    expired_ids = [tid for tid, t in _tasks_finish_time.items() if now - t > ttl_seconds]
    for tid in expired_ids:
        ...
        _tasks_finish_time.pop(tid, None)
...
# ---------------------------
# SSE emit 回调注册表（跨模块共享）
# ---------------------------
# 说明：不能放在 query_service.py 里！
# query_service.py 以 python xxx.py 启动时模块名是 __main__，
# 而节点里 from atguigu.web.api.query_service import 拿到的是另一个模块实例（空字典）。
# task_utils.py 导入路径唯一，所有模块共享同一份数据。
# key: task_id, value: emit(event, data) 回调函数
_emit_callbacks: Dict[str, Callable] = {}


def register_emit(task_id: str, emit: Callable) -> None:
    """注册 SSE 推送回调（exec_query_graph 调用）"""
    _emit_callbacks[task_id] = emit


def get_emit(task_id: str) -> Callable:
    """获取 SSE 推送回调（节点调用，可能返回 None）"""
    return _emit_callbacks.get(task_id)


def unregister_emit(task_id: str) -> None:
    """任务结束后清理回调，避免内存泄漏"""
    _emit_callbacks.pop(task_id, None)
