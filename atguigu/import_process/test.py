...
class NodeMDImg(NodeBase):
    ...
    def process(self, state: ImportGraphState):
        ...
        errors = minio_client.remove_objects(bucket_name = MinIoConfig.minio_bucket_name,delete_object_list=delete_image_list)

        for image_context_summary in image_context_summary_list:
            upload_image = minio_client.fput_object(bucket_name = MinIoConfig.minio_bucket_name,onject_name = upload_dir + '/' + image_context_summary['image_name'],file_path = image_context_summary['image_path'])
            logger.info(f'图片上传结果:{json_format(upload_image)}')
            url = f'http://'
